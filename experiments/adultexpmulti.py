...
def main():
    ...
    parser.add_argument("--model", type=str, default="linear", choices=["linear", "mlp", "rf"], help="Model type")
    parser.add_argument("--hidden_dim", type=int, default=50, help="Hidden dimension for MLP")
    ...
    scheduler = None
    lr = 1e-3 if args.model == "mlp" else 1e-2
    ...
    for trial in range(max_trials):
        ...
        def get_model():
            if args.model == "linear":
                return LinearNet(dataset.d, 3).to(device), 100
            elif args.model == "mlp":
                return MLP(dataset.d, args.hidden_dim, 3).to(device), 100
            elif args.model == "rf":
                # Train RF immediately using dataset train data
                # Extract numpy arrays from loader
                X_train, y_train = [], []
                ...
                return RFWrapper(clf, device), 0
            else:
                raise ValueError("Unknown model")

        logging.info(f"Trial {trial+1}/{max_trials} | Model: {args.model}")

        # Cost-optimized combination 
        model, epochs_run = get_model()
        plc = PL_Combine_Cost(model, device)
        plc.combiner.eps = 1e-12
        plc.fit(
        ...
        # Fair combination
        model, epochs_run = get_model()
        plc = PL_Combine_Fair(model, device, fairness_cost=20.0, human_cost=0.1)
        plc.combiner.eps = 1e-12
        plc.fit(
            dataset.data_train_loader,
            dataset.data_val_loader,
