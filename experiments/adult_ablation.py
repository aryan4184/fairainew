...
def main():
    ...
    THRESHOLD_RANGE = np.arange(0.0, 1.0, 0.09)
    TOTAL_EPOCHS = 100
    LR = 1e-2
    ...
    for K in K_VALUES:
        ...
        for r in THRESHOLD_RANGE:
            ...
            eqd_data.append(
                {
                    "threshold": r,
                    "EQD_c0": metrics_dict["system_equalized_odds_difference_c0"],
                    "EQD_c1": metrics_dict["system_equalized_odds_difference_c1"],
                    "EQD_c2": metrics_dict["system_equalized_odds_difference_c2"],
    ...
    for K, df in results.items():
        ...
        plot_path = os.path.join(PLOT_DIR, f"adulteqd_k{K}.png")
        plt.savefig(plot_path)
        plt.close()

        # Save CSV
        csv_path = os.path.join(PLOT_DIR, f"results_k{K}.csv")
