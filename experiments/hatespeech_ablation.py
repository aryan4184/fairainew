...
# Metric helper
def compute_eod_metrics(data, class_num=3):
    """
...
def main():
    ...
    TOTAL_EPOCHS = 50 # Hatespeech might need fewer or more, keeping reasonable default
    LR = 1e-2
    ...
    for K in K_VALUES:
        ...
        for r in THRESHOLD_RANGE:
            ...
            metrics_dict = compute_eod_metrics(output, class_num=3)
            ...
            eqd_data.append(
                {
                    ...
                    "EQD_c2": metrics_dict.get("system_equalized_odds_difference_c2", 0),
    ...
    for K, df in results.items():
        ...
        plt.title(f"Hatespeech EOD and Deferral Rate vs Threshold (K={K})")
        plt.xlabel("Fairness Cost (r)")
        plt.ylabel("EOD / Deferral Rate")
        plt.legend()
        plt.grid(True)

        plot_path = os.path.join(PLOT_DIR, f"hatespeech_eqd_k{K}.png")
        plt.savefig(plot_path)
        plt.close()

        # Save CSV
        csv_path = os.path.join(PLOT_DIR, f"hatespeech_results_k{K}.csv")
